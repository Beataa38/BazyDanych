...
-- 6b Modyfikacja i wyœwietlenie danych
	...
	--c wyœwietlenie danych pracownika z najd³u¿szym nazwiskiem
	SELECT id_pracownika, imiê, UPPER(nazwisko) AS nazwisko, adres, telefon 
		FROM ksiegowosc.pracownicy 
		WHERE LEN(nazwisko) = (SELECT MAX(LEN(nazwisko)) FROM ksiegowosc.pracownicy);

	--d wyœwietlenie danych pracowników i ich pensji (algorytm md5)
	SELECT pracownicy.id_pracownika, HASHBYTES('MD5', imiê) AS imiê, HASHBYTES('MD5', nazwisko) AS nazwisko, HASHBYTES('MD5', adres) AS adres, HASHBYTES('MD5', telefon) AS telefon,
	...
	--f Wyœwietlenie pracowników, pensji, premii (z³¹czenie lewostronne)
	SELECT pracownicy.id_pracownika, pracownicy.imiê, pracownicy.nazwisko, pensje.kwota AS pensja, premie.kwota AS premia 
		FROM ksiegowosc.pracownicy
	...
	SELECT CONCAT('Pracownik ', pracownicy.imiê, ' ', pracownicy.nazwisko, ', w dniu ', DAY(wynagrodzenie.data), '.0', MONTH(wynagrodzenie.data),'.', YEAR(wynagrodzenie.data), 
		' otrzyma³ pensjê ca³kowit¹ na kwotê ', FORMAT(pensje.kwota + premie.kwota, 'N0'), ' z³, gdzie wynagrodzenie zasadnicze wynosi³o: ', 
